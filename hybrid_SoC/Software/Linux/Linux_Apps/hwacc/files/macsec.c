...
static struct scatterlist *copy_scatterlist(struct scatterlist *sg, int nents)
{
    ...
    /* Allocate new scatterlist */
    new_sg = kmalloc_array(nents, sizeof(struct scatterlist), GFP_KERNEL);
    if (!new_sg)
        return NULL;

    /* Initialize scatterlist */
    sg_init_table(new_sg, nents);
    ...
    for (i = 0; i < nents; i++) {
        ...
        sg_copy_to_buffer(&sg[i], 1, data_buf, len);

        /* Set up new scatterlist entry */
        sg_init_one(&new_sg[i], data_buf, len);

        /* Link the scatterlist entries properly */
        if (i < nents - 1)
            sg_chain(&new_sg[i], 1, &new_sg[i + 1]);
    }

    /* Mark the last entry */
    sg_mark_end(&new_sg[nents - 1]);
...
cleanup:
    while (i-- > 0)
        kfree(sg_virt(&new_sg[i]));  // Free allocated buffers
    kfree(new_sg);
    return NULL;
}

/* Function to deep copy an aead_request */
struct aead_request *deep_copy_aead_request(struct aead_request *req, struct crypto_aead *tfm)
{
    struct aead_request *new_req;
    int src_nents = 0, dst_nents = 0;
    size_t req_size, iv_size;

    if (!req)
        return NULL;

    /* Calculate the size of aead_request including the flexible array */
    req_size = sizeof(*req) + crypto_aead_reqsize(tfm);

    /* Allocate memory for the new request */
    new_req = kmalloc(req_size, GFP_KERNEL);
    if (!new_req)
        return NULL;
    ...
    /* Get the IV size dynamically */
    iv_size = crypto_aead_ivsize(tfm);
    if (iv_size > 0) {
        new_req->iv = kmalloc(iv_size, GFP_KERNEL);
        if (!new_req->iv)
            goto free_new_req;
        memcpy(new_req->iv, req->iv, iv_size);
    }

    /* Copy scatterlists */
    if (req->src) {
        src_nents = sg_nents(req->src);
        new_req->src = copy_scatterlist(req->src, src_nents);
        if (!new_req->src)
            goto free_iv;
    }
    ...
    if (req->dst) {
        ...
        new_req->dst = copy_scatterlist(req->dst, dst_nents);
        if (!new_req->dst)
            goto free_src;
    }
				      ...
				      struct net_device *dev)
{
	...
	//new_req = deep_copy_aead_request(req, tx_sa->key.tfm);	

	ret = egress_thread_add_work(req, 1,0);
				      ...
				      struct macsec_secy *secy)
{
	...
	//new_req = deep_copy_aead_request(req, rx_sa->key.tfm);	

	ret = egress_thread_add_work(req, 0,1);
		      ...
		      int icv_len)
{
	...
	egress_thread_init_rx(sak, key_len);
		      ...
		      int icv_len)
{
	...
	egress_thread_init_tx(sak, key_len);

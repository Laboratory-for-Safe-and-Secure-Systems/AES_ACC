...
void init_tx_BRAM_FPGA(void) {
    ...
    // Map the physical memory to kernel virtual address space
    mapped_base_DR_tx = ioremap(page_base, PAGE_SIZE);
...
void init_rx_BRAM_FPGA(void) {
    ...
    // Map the physical memory to kernel virtual address space
    mapped_base_DR_rx = ioremap(page_base, PAGE_SIZE);
...
void init_rx_IV_FPGA(struct aead_request *req)
{
	...
	pr_notice("cryptlen bevor: 0x%llx \n", cryptlen);

	cryptlen=cryptlen-0x10;

	pr_notice("cryptlen after: 0x%llx \n", cryptlen);
...
void write_tx_payload_FPGA(struct aead_request *req)
{
	...
	write_to_memory_DR(mapped_base_DR_tx, DATA_READY_tx+8, 1);
}
...
void write_rx_payload_FPGA(struct aead_request *req)
{
	...
	write_to_memory_DR(mapped_base_DR_rx, DATA_READY_rx+8, 1);
...
int read_tx_payload_FPGA(struct aead_request *req) {
	...
	pr_notice("ICV Offset: 0x%llx ,Data: 0x%llx \n", icv_offset1,icv_data_1);
	pr_notice("ICV Offset: 0x%llx ,Data: 0x%llx \n", icv_offset2,icv_data_2);
    ...
    write_to_memory_DR(mapped_base_DR_tx, DATA_READY_tx + 8, 0);
    req->src = req->dst;
...
int read_rx_payload_FPGA(struct aead_request *req) {
    ...
    write_to_memory_DR(mapped_base_DR_rx, DATA_READY_rx + 8, 0);
    req->src = req->dst;
...
static void print_sg_list(struct scatterlist *sg, const char *label)
{
    size_t sg_offset = 0;
    uint64_t temp_buffer;

    pr_info("%s Scatterlist:\n", label);

    while (sg) {  // Iterate over the entire scatterlist
        uint8_t *sg_data = sg_virt(sg);

        for (sg_offset = 0; sg_offset < sg->length; sg_offset += sizeof(uint64_t)) {
            temp_buffer = 0;

            if (sg_offset + sizeof(uint64_t) > sg->length) {
                // If the remaining data is less than 8 bytes, copy only the available bytes
                memcpy(&temp_buffer, sg_data + sg_offset, sg->length - sg_offset);
            } else {
                memcpy(&temp_buffer, sg_data + sg_offset, sizeof(uint64_t));
            }

            pr_info("Offset: %lu, Data: 0x%llx\n", sg_offset, temp_buffer);
        }

        sg = sg_next(sg);  // Move to the next scatterlist entry
    }

    pr_info("Finished printing %s scatterlist\n", label);
}

void print_scatterlists(struct kritis3m_queue_element *work_item)
{
    if (!work_item || !work_item->req) {
        pr_err("Invalid work_item or request\n");
        return;
    }

    struct aead_request *req = work_item->req;

    pr_info("Printing scatterlists for AEAD request\n");

    print_sg_list(req->src, "Source");
}
		...
		list_for_each_safe(iterator, next, &WORK_LIST) 
		{
			...
			while ((crypting & 1) != 1) {
				...
				if (work_item->encrypt==true)
				{
					...
					crypting = read_from_memory_DR(mapped_base_DR_tx) & 1;  
				}
				if (work_item->decrypt==true)
				{
					
					crypting = read_from_memory_DR(mapped_base_DR_rx) & 1;  
				}
			}
